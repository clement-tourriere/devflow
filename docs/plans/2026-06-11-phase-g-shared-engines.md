...
| G3 shared Redis | ✅ done (2026-06-11) — `services/shared/redis.rs`, `SharedRedisProvider` for `service_type: redis` (always shared). One global `redis:7` container (AOF persistence, optional `--requirepass`); a numbered **DB index** (1–15) per workspace, allocated **atomically via a Redis Lua EVAL** and stored in a reserved hash (`devflow:dbindex` in DB 0) so Redis is self-describing. delete = FLUSHDB + release; connection string `redis://…/N`. **Hard cap: 15 workspaces across ALL projects** (Redis has 16 DBs, global) — clear error on exhaustion. 232 tests + clippy green. **Untested against a live Redis daemon.** |
...
| G5 daemon | 🟡 partial (2026-06-11) — the **reconcile primitive** is done: `factory::reconcile_shared_engines(config)` ensures every configured shared engine's global container is running (the loop body a daemon would run), plus `factory::is_shared_service()`, exposed as the **`devflow service up`** CLI command (JSON-aware, fails if any engine won't start). 4 unit tests (filtering + no-Docker path). **The full background daemon is not built** — see design below. |

### G5 remaining: the background controller daemon (design)

The reconcile primitive (`reconcile_shared_engines`) is the synchronous loop body. To become a true always-on controller, fold it into the long-running proxy process (`crates/devflow-proxy`), which already has: a Docker event stream + reconnect (`monitor.rs`, fixed in Phase C), a JSON API on 127.0.0.1 (`api.rs`), and pidfile/daemon lifecycle (`src/cli/proxy.rs`). Plan:
- On proxy start, read each registered project's config (the GUI project registry in `src-tauri/src/state.rs` should move into devflow-core so the daemon sees all repos) and call `reconcile_shared_engines` for each; re-run on a timer and on Docker `die` events for `devflow.scope=global` containers (auto-restart a downed engine).
- Add `/api/engines` to report engine health; add `devflow daemon status` reading it.
- Git-hook/CLI fast path: when the daemon is alive, `switch`/`create` POST to it to ensure engines instead of doing it in-process (same `reconcile_shared_engines` code, so no behavior divergence); fall back to in-process when the daemon is down.
