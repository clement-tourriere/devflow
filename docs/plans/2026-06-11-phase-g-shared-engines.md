...
| G5 daemon | ✅ done (2026-06-11) — `factory::reconcile_shared_engines` (one project) + `factory::reconcile_all_projects` (every registered project, the machine-wide loop body) + `factory::is_shared_service`. Two surfaces: **`devflow service up`** (reconcile once) and the **`devflow daemon start/stop/status`** background controller (`src/cli/daemon.rs`) — detached spawn + pidfile + JSON status file, timer-based reconcile loop, `--once`/`--foreground` modes. Reuses the proxy's daemonization pattern; lives in the main binary (proxy crate doesn't depend on core). **Validated end-to-end against real Docker**: registered a shared-redis project, stopped its container, `daemon start --once` reported `projects:1, engines_running:1` and **restarted the dead container**. |

**Daemon design notes (as built + future):** a standalone `devflow daemon` in the main binary was chosen over folding into the proxy crate (which doesn't depend on devflow-core). The reconcile is timer-based (default 30s) — auto-restarts a downed engine within one interval. `reconcile_all_projects` is correct regardless of the daemon's cwd because `test_connection`/`ensure_ready` key on each engine's fixed global container name, not per-project identity. Future optimizations (not needed for correctness): event-driven reconcile on Docker `die` events for `devflow.scope=global` containers; a `/api/engines` health surface; a git-hook fast path that POSTs to the daemon when alive.
